...
from Gates.b3m import b3_mona1
...
import re,time
@Client.on_message(filters.command("btm" , prefixes=["/", "."]))
async def chk(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('btm') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('btm ')) > 1 and message.text.split('btm ')[1].strip():
            ccc = message.text.split('btm ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await b3_mona1(cc,mm,yy,cvv)
    brand , type, level, bank, country, emoji = await bin_data(cc[:6])
    rank = await get_user_rank(user_id)
    end_time = time.perf_counter()
    elapsed_time = f"{end_time - start_time:.2f}"
    if 'Nice! New payment method added' in msg:
        status = '𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅'
        msg = '1000: Approved (M S APPROVED * = 027)'
    elif 'Duplicate card exists in the vault.' in msg:
        status = '𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅'
        msg = 'Approved - Duplicate'
    elif 'Insufficient Funds' in msg or 'Card Issuer Declined CVV (483 : DECLINED * REFER CALL TO ISSUE=)' in msg:
        status = '𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅'
    else:
        status = '𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌'
    text = f'''
<b>#Braintree_Moneris_Auth 🔥 [/btm]
- - - - - - - - - - - - - - - - - - - - - - - -</b>
